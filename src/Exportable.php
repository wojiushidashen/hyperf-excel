 ...
 * @property \Hyperf\Database\Model\Collection $data
 */
trait Exportable
{
    /**
    ...
    private function exportOrDownload($path, $function, callable $callback = null)
    {
        ...
        foreach ($data as $key => $collection) {
            if ($collection instanceof Collection || $collection instanceof \Hyperf\Utils\Collection) {
    ...
    private function writeRowsFromCollection($writer, $collection, ?callable $callback = null)
    {
        // Apply callback
        if ($callback) {
